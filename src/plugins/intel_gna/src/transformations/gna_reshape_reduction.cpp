...
GnaReshapeToSqueeze::GnaReshapeToSqueeze() {
...
GnaReshapeToUnsqueeze::GnaReshapeToUnsqueeze() {
...
GnaReshapeFuse::GnaReshapeFuse() {
    ...
    ov::matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        auto input_shape = reshape_node_new->get_input_shape(0);
        auto output_shape = reshape_node_new->get_output_shape(0);
        std::shared_ptr<ov::Node> input_parent = reshape_in_node->get_input_node_shared_ptr(0);
        if (graph_utils::are_shapes_equal(input_shape, output_shape)) {
            ov::replace_output_update_name(reshape_out_node->output(0), input_parent->output(0));
        } else {
...
bool GnaReshapeReduction::run_on_model(const std::shared_ptr<ov::Model>& m) {
    RUN_ON_MODEL_SCOPE(ReshapeReduction);

    ov::pass::Manager manager(get_pass_config());
    manager.register_pass<GnaReshapeFuse>();
    manager.register_pass<GnaReshapeToSqueeze>();
    manager.register_pass<GnaReshapeToUnsqueeze>();
