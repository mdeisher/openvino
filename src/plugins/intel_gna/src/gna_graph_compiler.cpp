...
#    define GET_ACTIVATION_NAME(name) \
    ...
    if (!gna_config.gnaFlags.sw_fp32) {
        ...
        if (gna_config.gnaFlags.uniformPwlDesign) {
            ...
            //printf("activation %s inscale %f outscale %f\n", layer->name.c_str(), input_pwl_scale_factor, output_pwl_scale_factor);
            ...
            PwlDesignOpt(activation_type,
                         ...
                         ptr_pwl_segments);
            //for (auto i = 0; i < ptr_pwl_segments.size(); i++) {
            //    printf("   slope %d xbase %d ybase %d\n", ptr_pwl_segments[i].slope, ptr_pwl_segments[i].xBase, ptr_pwl_segments[i].yBase);
            //}
        }
        ptr_pwl_segments_target = reinterpret_cast<gna_pwl_segment_t*>(&ptr_pwl_segments_target);
    }
